...
function ProjectItem(props) {
  return (
    <div className="group flex flex-col  p-5 transition-all bg-surface-200 hover:scale-110 brightness-75 hover:brightness-100 hover:z-10">
      ...
      <h1 className="mb-4 text-xl subpixel-antialiased">
        <Link href={`${props.hrefprod}`}>{props.name}</Link>
      </h1>
      <div className="text-surface-600 text-xs mb-2">
        <Link href={`${props.href}`}>код</Link>
      </div>
